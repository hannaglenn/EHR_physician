...
For those who remain practicing, different settings or locations of practice could be in the choice set. Specifically, if EHRs change job satisfaction of working in the hospital, a hospitalist may decide to shift more of their working hours towards or away from the hospital. This is relevant for hospitalists who contract time to hospitals, as not all physicians working as hospitalists have a binding contract with their associated hospital. A 2013 survey estimated that 36\% of hospitalists also worked outside of the hospital in primary care, long-term care, or hospice settings (\cite{Today}). Many physicians trained as internists choose to work in offices and also spend part of their time in inpatient settings. The fraction of internists who work as hospitalists increased from 25\% to 40\% from 2008 to 2018 (\cite{gray2022evolving}).  

For hospitalists who do have a binding contract with a hospital, there could still be opportunities to shift work from inpatient to outpatient settings owned by the hospital. Particularly, if EHRs change hospital incentives of where to allocate physicians, EHR adoption could affect work setting in a way that is outside of the hospitalists' control. The sample of physicians I consider include both hospitalists with formal contracts, and internists who contract a majority of their time in the hospital inpatient setting, but could also work in hospital outpatient settings or independent offices. Medicare billing codes are location specific, so for each physicians I observe claims filed in offices, in outpatient settings, and in inpatient settings. Prior to 2016, office-based claims can be chosen for physicians in independent practices or in off-campus hospital outpatient departments. However, physicians working in off-campus hospital outpatient departments have strong incentive to claim as an outpatient department because the reimbursement rate is much higher (\cite{post2021hospital}). Therefore, I treat office-based claims as independent practices and investigate these separately from hospital outpatient departments. Further, I investigate whether these groups differentially change work setting due to EHR through a heterogeneity analysis of hospitalists working in hospitals with different levels of integration with physicians.
...
\subsection{Other Work Environment Changes}\label{sec:hospanalysis}
...
A potential concern is that hospitalists are not leaving a clinical setting, but choosing to only see patients with private insurance. First, EHR use should not be directly correlated with Medicare patients. Further, as a hospitalist, these physicians do not have complete control over their patient composition and likely are not paid differently based on the types of patients they see. As a final check, the CMS publishes a list of physicians who have officially opted out of seeing Medicare patients and none of the hospitalists in the sample have selected into this option. 

It could also be that physicians are not necessarily choosing to exit clinical settings, but that hospitals are less willing to continue employment of physicians that are inefficient in EHR use. However, if this is the case, then one would expect to find differing results depending on the type of contract the hospitalist has with the employing hospital. That is, hospitalists with more integrated relationships would show a higher likelihood of exiting clinical work after EHR, as the hospital has more control over their employment. I show in Section \ref{sec:mechanisms} that this is not driving the main results. 


\subsubsection{Evaluating Pre-Trends}

When analyzing the effect of EHRs on exiting clinical settings, there is no visual indication of a violation of the parallel trends assumption in the pre-periods. However, the p-value of a Wald test for pre-trends indicates some evidence for such a violation for younger hospitalists. In Appendix \ref{sec:pretrends}, I present confidence intervals robust to various specifications of a violation in parallel trends. The estimate becomes null with any specified variation in the parallel trend assumption. Thus, I focus on the results for older hospitalists. I also present overall treatment effects under different data specifications for this outcome in Appendix \ref{sec:changes}. Under various data and specification changes, the coefficients remain non-negative. Estimates become noisier under strict limitations of the data, but given the rarity of exiting, this positive effect still seems to be robust. 

I also investigate pre-trends for switching to office in Appendix \ref{sec:pretrends}, where I find that under various assumptions of the form of violation in parallel trends, the estimate remains positive and significant of similar magnitude. Further, under different combinations of data specifications I find consistently positive and significant results, as shown in Appendix \ref{sec:changes}. 

\subsubsection{Lee Bounds for Sample Selection}
...
\subsubsection{Change in Patient Distribution}
...
\subsection{Effect of EHR Exposure on Work in Outpatient Settings}

Analyzing the effect of EHR exposure on patients seen in independent office settings reveals that hospitalists are shifting work towards a setting where they may have more control over their work environment. However, there are likely hospitalists in my sample for which is it more feasible to shift work between inpatient and outpatient hospital settings. Therefore, I present estimates of the effect of EHR exposure on fraction of patients seen in outpatient hospital departments in Figure \ref{fig:outpat}. Here, outpatient departments can be either on or off the hospital campus. Hospitalists increase the fraction of patients seen in outpatient settings by up to .01 ppts in each year after EHR exposure. This magnitude is much smaller than that for independent office visits, potentially indicating less choice of work setting by these hospitalists. 

    \begin{figure}[ht!]
\centering
    \captionsetup{width=.85\linewidth}
    \caption{Effect of EHR Exposure on Fraction of Patients Seen in Outpatient}
   \begin{subfigure}[]{.8\textwidth}
   \caption{all physicians}
   \includegraphics[scale=.5]{Objects/outpat_all.pdf}
   \label{fig:outpat_all} 
\end{subfigure}
...
\begin{subfigure}[]{.8\textwidth}
\caption{sub-samples of physicians based on age}
   \includegraphics[scale=.5]{Objects/oupat_ages.pdf}
   \label{fig:outpat_ages}
\end{subfigure}
\label{fig:outpat}
\vspace{2mm}
    \caption*{\footnotesize{\textit{Notes:} The top panel shows average group time treatment effects aggregated over groups to an event study plot. The bottom show these results for different subgroups of physicians by age. The p-value listed for each graph corresponds to a Wald test for pre-trends. Confidence intervals shown are simultaneous confidence bands accounting for multiple hypothesis testing. Overall ATT for all, $<$ 60, $>=$ 60 with SE in parentheses: .008 (0.002), .009 (0.003), 0.006 (0.007), respectively.}}
\end{figure}

\subsubsection{Mechanisms}\label{sec:mechanisms}

As discussed in Section \ref{sec:worksetting}, hospitalists could be switching work setting either because of (1) their own preferences to avoid technology (as outpatient department EHRs tend to look different than inpatient EHRs), (2) because EHRs, through improved substitution between work settings, cause hospitals to re-allocate workers to hospital-owned outpatient settings, or because (3) physicians have always preferred to see patients in outpatient settings, but before EHRs certain tasks could only be performed in inpatient settings due to efficiency or patient sharing. In this section, I specifically explore the role of hospital re-allocation of work vs. physician preferences, as I cannot distinguish observed physician behavior from which preference might be driving that behavior. 

I do not observe individual physician contract type with each hospital, but I do observe hospital level information about the aggregate nature of their physician contracts. To investigate the potential mechanism of hospital allocation, I present a heterogeneity analysis by level of integration the physician is exposed to in their connected hospitals. From least integrated (contracts with multiple facilities) to most integrated (full salaried hospitalists), results are broken down by Independent Practice Association (IPA), Open Physician-Hospital Organization (OPHO), Closed Physician-Hospital Organization (CPHO), and Integrated Salary Model (ISM). In practice, hospitals drive the re-allocation of work setting to outpatient only if the hospitalist is heavily integrated. Further, in order for a hospital to have an outpatient facility to re-allocate physicians to, they must be heavily integrated with the physicians currently in that outpatient facility. For contracted hospitalists, the choice remains whether to contract with a hospital who wants them to work in an outpatient setting. However, I present estimates of a heterogeneity analysis for fraction of patients in outpatient settings by integration level in Figure \ref{fig:het_int}, which shows that the results are not driven entirely by integrated physicians. 

\begin{figure}[ht!]
    \centering
    \caption{Effect of EHR Exposure on Outcomes by Integration Level}
    \includegraphics[width=.6\textwidth]{Objects/heterog_plot_int_office.pdf}
    \label{fig:het_int}
\end{figure}

Another proxy for contract type is whether hospitalists work with one system or are connected to facilities from different systems. I present the results from this heterogeneity analysis in Appendix \ref{sec:het}, and find no difference in effects between the two groups. Finally, I investigate whether this finding is potentially due to EHRs changing the type of billing performed by hospitalists. For example, they could be performing the same services in the same location, but coding the billing information differently. I present this analysis in Appendix \ref{app:coding}, and I do not find this to be the case.
...
\begin{figure}[ht!]
    \centering
    \captionsetup{width=.6\linewidth}
    \caption{Effect of EHR Exposure on Claims per Patient by Group}
    \includegraphics[scale=.4]{Objects/claim_group.pdf}
    \label{fig:patientgroup}
    \vspace{2mm}
    \caption*{\footnotesize{\textit{Notes:} Plot shows average group time treatment effects for each treatment group.}}
\end{figure}
