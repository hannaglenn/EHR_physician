...
/* Import Physician Level Dataset */
 ...
 xteventplot , nopostpval scheme(s1mono) ylabel(, grid angle(0) labsize(small)) xlabel(, labsize(small)) ytitle("Estimate and 95% CI" " ", size(small)) xtitle(" " "Event Time", size(small)) title("Event Study, Dep. Variable: Number Patients (Full Sample)" " ", size(medsmall))
 ...
 xteventplot , nopostpval scheme(s1mono) ylabel(, grid angle(0) labsize(small)) xlabel(, labsize(small)) ytitle("Estimate and 95% CI" " ", size(small)) xtitle(" " "Event Time", size(small)) title("Event Study, Dep. Variable: Number Patients (Old Sample)" " ", size(medsmall))
